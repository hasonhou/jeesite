...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bussiness/program/toProgramList?category.id=45b559276fc8434f9a074b7db45f0636">项目列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/bussiness/program/toProgramList" method="post" class="breadcrumb form-search">
		...
		<label>项目名称：</label><form:input path="title" htmlEscape="false" maxlength="50" class="input-small"/>&nbsp;
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><!-- <th>栏目</th> --><th>项目名称</th><!-- <th>权重</th> --><th>点击数</th><th>发布者</th><th>更新时间</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="article">
			<tr>
				...
				<td><a href="${ctx}/bussiness/program/toEditPage?id=${article.id}&category.id=${article.category.id}" title="${article.title}">${fns:abbr(article.title,40)}</a></td>
				...
				<td>
					...
					<shiro:hasPermission name="bussiness:program:edit">
	    				...
	    				<a href="${ctx}/bussiness/program/toEditPage?id=${article.id}">修改</a>
	    				<shiro:hasPermission name="cms:article:audit">
							<a href="${ctx}/bussiness/program/delete?id=${article.id}${article.delFlag ne 0?'&isRe=true':''}&categoryId=${article.category.id}" onclick="return confirmx('确认要${article.delFlag ne 0?'发布':'删除'}该文章吗？', this.href)" >${article.delFlag ne 0?'发布':'删除'}</a>
						</shiro:hasPermission>
					</shiro:hasPermission>
				</td>
			</tr>
